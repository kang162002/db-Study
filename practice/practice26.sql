...
DELETE FROM 테이블명;      
DML 데이터 삭제 조건으로 원하는 데이터만 삭제(WHERE 특정 데이터 선택)

TRUNCATE TABLE 테이블명;    
DLL 데이터 삭제 전체 데이터 바로 삭제 삭제시 바로 적용(auto commit)
...
VALUES (T_MEMBER_POINT_PK_SEQ.nextval,(SELECT COUNT(*)+1 FROM T_MEMBER_POINT WHERE member_id = 'A'),
        'A', 30, TO_DATE('2025-05-30') );
        
        
        INSERT INTO T_MEMBER_POINT (id,no,member_id,point,check_date)
VALUES (T_MEMBER_POINT_PK_SEQ.nextval,(SELECT COUNT(*)+1 FROM T_MEMBER_POINT WHERE member_id = 'B'),
        'B', 30, TO_DATE('2025-05-30') );
        
        
        INSERT INTO T_MEMBER_POINT (id,no,member_id,point,check_date)
VALUES (T_MEMBER_POINT_PK_SEQ.nextval,(SELECT COUNT(*)+1 FROM T_MEMBER_POINT WHERE member_id = 'C'),
        'C', 30, TO_DATE('2025-05-30') );
        
        
        INSERT INTO T_MEMBER_POINT (id,no,member_id,point,check_date)
VALUES (T_MEMBER_POINT_PK_SEQ.nextval,(SELECT COUNT(*)+1 FROM T_MEMBER_POINT WHERE member_id = 'A'),
        'A', 50, TO_DATE('2025-05-30') );
